...
def get_receive_program(user_id,amount):
    ...
    # return amount_receve
def FnRefferalProgram(user_id,package):
...
def ThunhapF1(user_id,amount):
    ...
    if customer['p_node'] != '0' or customer['p_node'] != '':
        ...
        if customer_p_node is None:
            ...
            percent = 5
            commission = float(amount)* percent /100
            ...
            detail = 'Nhận %s VNĐ từ %s thu nhập f1 của ID %s. (%s VNĐ)' %("{:20,.0f}".format(commission),str(percent)+'%',username_invest,"{:20,.0f}".format(amount))
            SaveHistory(customer_p_node['customer_id'],
                customer_p_node['username'], 
                commission, 
                'thunhapF1',  
...
def AdminAddCustomerSubmit():
    ...
    if request.method == 'POST':
        ...
        if package == 10000000:
            monthly = package * 2.5/100
            cophieu = 100
        if package == 30000000:
            monthly = package * 3/100
            cophieu = 300
        if package == 50000000:
            monthly = package * 3.5/100
            cophieu = 500
        if package == 100000000:
            monthly = package * 4/100
            cophieu = 1000
        if package == 300000000:
            monthly = package * 4.5/100
            cophieu = 3000
        if package == 1890000000:
            monthly = package * 5/100
            cophieu = 20000
        if package == 3780000000:
            monthly = package * 5.5/100
            cophieu = 40000
...
def ActiveInvest(customer_id):
    ...
    return render_template('admin/active-invest-select.html', data=data)
...
def ActiveInvestSubmit(customer_id):
    ...
    if int(customer['active']) == 0:
        ...
        db.deposits.insert(data_deposit)
        FnRefferalProgram(customer_id,package)
        db.users.update({ "_id" : ObjectId(customer['_id']) }, { '$set': {'active' :2,'cophieu' : cophieu} })
...
def ProfitDailySubmit():
    ...
    for x in listdeposit:
        ...
        ThunhapF1(customer['customer_id'],float(x['monthly']))
...
def CancapSubmit():
    ...
    for x in listcustomer:
        ...
        if int(x['count_leftss']) > int(x['count_rightss']): 
            amount_receve = int(x['count_rightss'])*0.05
            cancap =  int(x['count_rightss'])
        else:
            amount_receve = int(x['count_leftss'])*0.05
            cancap =  int(x['count_leftss'])
        #amount_receve = get_receive_program(x['customer_id'],amount_receve)


        if float(amount_receve) > 1000000000:
            amount_receve = 1000000000
